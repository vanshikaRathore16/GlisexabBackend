...
export const createUser = async(request,response,next)=>{
    try{
        ...
        return response.status(201).json({msg : "User created successFully! Check your mail for verify email"});
    }catch(err){
        console.log(err);
...
export const loginUser = async(request,response,next)=>{
    ...
    if(!user.isVerified)
        return response.status(401).json({err : "Please verify your email first"});
...
export const verifyAccount = async(request,response,next)=>{
    try{
        ...
        if(!email)
            ...
            return response.status(200).json({msg : "Account Verified Successfully! You can now login"});
    }catch(err){
        console.log(err);
        return response.status(500).json({err : "Internal server error"});
    }
